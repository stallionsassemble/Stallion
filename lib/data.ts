export const cardData = [
  {
    id: 1,
    sections: [
      {
        name: "Simplify Complex Ideas",
        description: "Break down blockchain concepts into digestible, engaging threads for diverse audiences."
      },
      {
        name: "Educate and Engage",
        description: "Craft compelling narratives that onboard newcomers and spark discussions."
      },
      {
        name: "Amplify Community Voices",
        description: "Share user stories and community achievements to foster connection and growth"
      },
      {
        name: "Drive Awareness",
        description: "Create viral-worthy threads that boost visibility for Stallion and the Stellar ecosystem."
      }
    ],
    tag:"Writers",
    imageURL: "/stallion-writer.png"
  },
  {
    id: 2,
    sections: [
      {
        name: "Design Seamless Interfaces",
        description: "Create intuitive, user-friendly designs that make blockchain technology accessible to all."
      },
      {
        name: "Champion Accessibility",
        description: "Design solutions that are inclusive and usable by diverse audiences globally."
      },
      {
        name: "Drive Innovation",
        description: "Experiment with emerging trends to craft visionary designs that set new standards in Web3."
      },
      {
        name: "Elevate User Experiences",
        description: "Optimise workflows and interactions to ensure products are delightful and impactful."
      }
    ],
    tag:"UI/UX DESIGNER",
    imageURL: "/stallion-designer.png"
  },
  {
    id: 3,
    sections: [
      {
        name: "Build Scalable Solutions", 
        description: "Develop cutting-edge applications and tools that empower users in emerging regions."
      },
      {
        name: "Enhance the Ecosystem",
        description: "Create integrations and features that expand the capabilities of the Stellar network."
      },
      {
        name: "Collaborate on Projects",
        description: "Join forces with creatives and operators to bring visionary ideas to life."
      },
      {
        name: "Contribute Open Source",
        description: "Share your code and ideas to strengthen the global developer community."
      },
    ],
    tag:"Web3 Developer",
    imageURL: "/stallion-developer.png"
  },
  {
    id: 4,
    sections: [
      {
        name: "Craft Engaging Campaigns",
        description: "Design compelling content strategies to spotlight emerging projects and initiatives."
      },
      {
        name: "Educate and Inspire", 
        description: "Create informative articles, videos, and tutorials to onboard newcomers to the Stellar blockchain."
      },
      {
        name: "Amplify Voices",
        description: "Share success stories and amplify the reach of community-driven innovations."
      },
      {
        name: "Visual Storytelling",
        description: "Develop eye-catching visuals and infographics that bring complex ideas to life"
      }
    ],
    tag:"Content Creator",
    imageURL: "/stallion-content.png"
  }
 ];

 export const howData = [
  {
    id: 1,
    title:"APPLY BOUNTY",
    sections: [
      {
        name: "Build Scalable Solutions", 
        description: "Develop cutting-edge applications and tools that empower users in emerging regions."
      },
      {
        name: "Enhance the Ecosystem",
        description: "Create integrations and features that expand the capabilities of the Stellar network."
      },
      {
        name: "Collaborate on Projects",
        description: "Join forces with creatives and operators to bring visionary ideas to life."
      },
    ],
  },
  {
    id: 2,
    title:"SUBMISSION",
    sections: [
      {
        name: "Build Scalable Solutions", 
        description: "Develop cutting-edge applications and tools that empower users in emerging regions."
      },
      {
        name: "Enhance the Ecosystem",
        description: "Create integrations and features that expand the capabilities of the Stellar network."
      },
      {
        name: "Collaborate on Projects",
        description: "Join forces with creatives and operators to bring visionary ideas to life."
      },
    ],
  },
  {
    id: 3,
    title:"Get Paid",
    sections: [
      {
        name: "Craft Engaging Campaigns",
        description: "Design compelling content strategies to spotlight emerging projects and initiatives."
      },
      {
        name: "Educate and Inspire", 
        description: "Create informative articles, videos, and tutorials to onboard newcomers to the Stellar blockchain."
      },
      {
        name: "Amplify Voices",
        description: "Share success stories and amplify the reach of community-driven innovations."
      },
    ],
  },
 ]  